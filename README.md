...
- passphrase-encrypted full-library backups containing charts, sources, and saved versions, with authenticated merge-only restore;
...
- a local-only Electron wrapper, checksum-verified setup/update scripts, automated desktop smoke checks, and a one-page macOS PDF guide.
...
The desktop renderer is prevented from making non-loopback network requests, so the application itself has no upload path to GitHub or another remote service. The working D1/R2 data folder remains outside the repository. `.gitignore`, a tracked-file scanner, and repository `pre-commit`/`pre-push` hooks reject database files, Worker data folders, encrypted backup packages, source-evidence documents, chart-shaped structured data, credentials, known personnel examples, and user-specific filesystem paths before normal Git operations. The push hook checks both the current index and reachable Git history, so data committed by bypassing the commit hook is still caught before a normal push. `scripts/setup-mac-source-test.zsh` enables those hooks, and `npm run security:scan` checks the complete tracked index.
...
Codex can continue helping after the desktop source test is installed, but it is not connected to the app in the background. The supported handoff uses reviewed local files and preserves the app's validation and history:
...
4. For layout-only changes, or when explicitly requested, Codex can operate the local Electron interface while it is running. It should use the app UI and import/export paths instead of editing the D1 storage files directly.

This keeps human confirmation, structural validation, autosave, named versions, and backups in the normal workflow. See [docs/CODEX-HANDOFF.md](docs/CODEX-HANDOFF.md) for the exact exchange pattern and boundaries.

## Encrypted backup and recovery

The **Sources & imports** workspace can create a portable recovery file containing every chart, layout, named version, source record, and available original imported file. The browser encrypts the complete package with AES-256-GCM using a key derived from a passphrase with PBKDF2-SHA-256. In the desktop app, it is then written directly to the selected backup folder; without a selected folder, it uses the normal browser download. The passphrase is not stored or recoverable by the application; keep it in an approved password manager, separate from the backup file.

Restore is intentionally merge-only:

1. Choose the `.orgchart-backup` file and enter its passphrase.
2. Authenticated decryption detects a wrong password or modified ciphertext.
