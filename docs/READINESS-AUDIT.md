...
| Backup and restore | AES-256-GCM passphrase backup of charts, sources, and versions; separate user-selected backup folder, including approved OneDrive/Dropbox locations; authenticated, merge-only restore | Crypto tamper/wrong-password tests, encrypted-folder write smoke, and merge-only Electron persistence smoke |
