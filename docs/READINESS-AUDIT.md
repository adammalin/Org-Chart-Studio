...
| Backup and restore | AES-256-GCM encryption by default or explicitly confirmed unencrypted backup of charts, sources, versions, and retained AI review decisions; either format may use a separate local or cloud-synced backup destination; configured-folder Run backup now action; merge-only restore; device-local last-backup/restore health and configurable reminders without passphrase storage | Crypto tamper/wrong-password tests, encrypted and unencrypted cloud-folder tests, unencrypted local-folder tests, schema compatibility tests, and merge-only Electron persistence smoke; reminder timing remains a human UI check |
